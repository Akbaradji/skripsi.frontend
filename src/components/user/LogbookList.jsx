...
export default function LogbookList() {
  ...
  const handleDelete = async (id, status) => {
    if (status !== 'pending') {
      toast.warn('Logbook yang sudah diproses tidak bisa dihapus.', { position: "top-right" });
      return;
    }
    ...
    return (
      <div className="pengajuan-list-container">
        <div className="pengajuan-list-card">
          ...
          {/* ⭐ PERBAIKAN: Menggunakan justify-content: space-between di CSS */}
          ...
          <table className="pengajuan-table">
            ...
            <tbody>
              {logbooks.map((logbook) => {
                const statusDisplay = getStatusDisplay(logbook.status);
                return (
                  <tr key={logbook.id} className="table-row">
                    ...
                    <td className="table-cell action-cell"> 
                      <div className="action-buttons-container">
                        ...
                        <button
                          onClick={() => handleDelete(logbook.id, logbook.status)}
                          disabled={logbook.status !== 'pending'}
                          className={`action-button delete-button ${logbook.status !== 'pending' ? 'disabled-button' : ''}`}
                          title={logbook.status !== 'pending' ? 'Logbook yang sudah diproses tidak bisa dihapus' : 'Hapus logbook'}
                        >
                          Hapus
                        </button>
                      </div>
